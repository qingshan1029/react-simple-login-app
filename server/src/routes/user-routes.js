...
routes.post('/signup', async (req, res) => {
    ...
    // console.log(fieldsInModel)
    // console.log(newUser.email)
    // console.log(isAdditionAllowed)
    ...
    try {
        ...
        Users.findOne({email : newUser.email}, function (error, user1) {
            if (user1) {
                return res.status(400).send({ error: 'The email is already existence' })
            }
            else
            {
                user.save()
